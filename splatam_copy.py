...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        #TODO apply motion prior gradient ascent correction here
...
# ---------------- L added ----------------
def correct_with_motion_prior(prior_integration, prior_config, params, time_idx, candidate_cam_unnorm_rot, candidate_cam_tran, device, lambda_step=0.1):
    with torch.no_grad():
        # Get past poses (10 frames)
        start_idx = max(0, time_idx - prior_config.past_frames)  
        past_rotations = params['cam_unnorm_rots'][..., start_idx:time_idx]  # [1, 4, T]
        past_translations = params['cam_trans'][..., start_idx:time_idx]     # [1, 3, T]

        
        past_rotations = past_rotations.permute(0, 2, 1)    # [1, T, 4]
        past_translations = past_translations.permute(0, 2, 1)  # [1, T, 3]
        # print('past_rotations', past_rotations.shape, past_rotations)
        # print('past_translations', past_translations.shape, past_translations)

        # Combine into poses: [1, 1, T, 7] where 7 = 3 (translation) + 4 (rotation)
        past_poses = torch.cat([
            past_translations.unsqueeze(1),  # [1, 1, T, 3]
            past_rotations.unsqueeze(1)        # [1, 1, T, 4]
        ], dim=-1)  # [1, 1, T, 7]


        ### ===== Relativise in the same way as in motion prior code ======
        # so T-1 should be [0., 0., 0., 0., 0., 0., 1.]
        # print('\n', past_poses[0,0,-1, :], '\n')


        # Process past poses for diffusion model
        batch_size, traj_mask, past_traj = prior_integration.data_preprocess_past_poses(prior_config, past_poses, device)

        # print(f"Processed:\n{past_traj[0,:,:]}")        # [1, T, D+3] third dim: [past_traj_abs, past_traj_rel, past_traj_vel]
        # exit()

        # Current SLAM pred (keep rotation as is)
        if candidate_cam_unnorm_rot.dim() == 1:
            slam_rot = candidate_cam_unnorm_rot.unsqueeze(0)  # [1, 4]
        else:
            slam_rot = candidate_cam_unnorm_rot  # [1, 4]
            
        # print('candidate_cam_tran:', candidate_cam_tran.shape)   # e.g., [1, 3]
        # print('slam_rot:', slam_rot.shape)  # e.g., [1, 4]

        slam_pose = torch.cat([candidate_cam_tran, slam_rot], dim=-1)  # [1, 7]
        # print(slam_pose.shape)

        # Generate samples using diffusion model
        sample_prediction, mean_estimation, variance_estimation = prior_integration.model_initializer(past_traj, traj_mask)
        ...
        initializer_preds = sample_prediction + mean_estimation[:, None]
        k_alternative_preds = prior_integration.p_sample_loop_accelerate(past_traj, traj_mask, initializer_preds)
        
        # Create KDE from the samples (for the immediate next timestep)
        k_preds_pose_for_kde = k_alternative_preds[0, :, 0, :].cpu().numpy()  # [K, 7]
        # k_preds_translation_for_kde = k_alternative_preds[0, :, 0, :3].cpu().numpy()  # [K, 3]  # Option 1 Translation only

        # print('k_preds_translation_for_kde', k_preds_translation_for_kde, k_preds_translation_for_kde.shape)
        prior = gaussian_kde(k_preds_pose_for_kde.T)


        # Apply gradient correction to translation only
        # slam_translation_np = candidate_cam_tran.detach().cpu().numpy()[0, :]  # shape: (3,)  # Option 1 Translation only
        slam_full_pose_np = slam_pose.detach().cpu().numpy()[0, :]  # shape: (7,)

        grad = prior_integration.prior_gradient_logl(prior, slam_full_pose_np)
        # grad = prior_integration.prior_gradient_logl(prior, slam_translation_np)

        
        # corrected_translation_np = slam_translation_np + lambda_step * grad
        corrected_pose_np = slam_full_pose_np + lambda_step * grad

        print('slam_full_pose_np:', slam_full_pose_np, '\corrected_pose_np:', corrected_pose_np)

        # Convert back to torch tensor
        # corrected_translation = torch.from_numpy(corrected_translation_np).to(device).unsqueeze(0)  # [1, 3]
        corrected_pose = torch.from_numpy(corrected_pose_np).to(device).unsqueeze(0)  # [1, 7]
        # print(corrected_pose.shape)

        # Keep rotation unchanged
        corrected_translation = corrected_pose[:, :3]  # [1, 3]
        corrected_rot = corrected_pose[:, 3:]  # [1, 4]

        return corrected_rot, corrected_translation
...
def rgbd_slam(config: dict):
    ...
    ##### Init prior methods
    config_dict = { #get dynamically from config file eg 6_Integration_10_5_50.yml
        "checkpoint_path": "./models/_pedestrian_prior_10_15_7D.pth",
        "traj_scale": 5.0,
        "past_frames": 10,
        "future_frames": 15,
        "dimensions": 7,
    ...
    #prior_integration.load_checkpoint("models/_newer_10_5_50_best_checkpoint_epoch_53.pth") #prior_config.checkpoint_path)
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            with torch.no_grad():
                ...
                ###motion prior implementation - uses past 10 poses
                if time_idx >= prior_config.past_frames:
                    corrected_rot, corrected_tran = correct_with_motion_prior(prior_integration, prior_config, params, time_idx, candidate_cam_unnorm_rot, candidate_cam_tran, device, lambda_step=0.01)
                    
                    # print('candidate_cam_unnorm_rot, candidate_cam_tran', candidate_cam_unnorm_rot, candidate_cam_tran)
                    # print('corrected_rot, corrected_tran', corrected_rot, corrected_tran)
