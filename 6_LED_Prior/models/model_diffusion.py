...
class TransformerDenoisingModel(Module):
	...
	def __init__(self, t_h: int, d_f:int, context_dim=256, tf_layer=2): #d_f = dimensions
		...
		if d_f in [2,3]:
			self.d_h = 2 * d_f  # 2D/3D: relative + velocity
		elif d_f in [6,7,9]:
			self.d_h = d_f + 3    # TUM: relative pose (7) + translation velocity (3) ### Lie: relative pose (6) + translation velocity (3) ### 6D: relative pose (9) + translation velocity (3)
		else:
			raise ValueError("Unsupported feature dimension - 6 and 9 DOF to be implemented")
		self.encoder_context = social_transformer(t_h, self.d_h)
		self.pos_emb = PositionalEncoding(d_model=2*context_dim, dropout=0.1, max_len=100) #TODO set to more for non-NBA
		...
		if d_f in [2, 3]:
			...
			# For 7D: treat it as translation (3) plus quaternion (4)
			self.linear_trans = ConcatSquashLinear(context_dim // 2, 3, context_dim + 3)
			self.linear_quat = ConcatSquashLinear(context_dim // 2, 4, context_dim + 3)
	...
	def forward(self, x, beta, context, mask):
		...
		if self.cfg.dimensions in [2,3]:
			...
			pred_trans = self.linear_trans(ctx_emb, trans)
			pred_quat = self.linear_quat(ctx_emb, trans)
			pred= torch.cat([pred_trans, pred_quat], dim=-1)
			...
			raise ValueError("Unsupported SE3 dimension in forward")
	...
	def generate_accelerate(self, x, beta, context, mask):
		...
		if dims in [2, 3]:
			...
			pred_trans = self.linear_trans.batch_generate(ctx_emb, trans_emb)
			pred_quat = self.linear_quat.batch_generate(ctx_emb, trans_emb)
			pred = torch.cat([pred_trans, pred_quat], dim=-1)
		elif dims == 9:
			pred_trans = self.linear_trans.batch_generate(ctx_emb, trans_emb)
			pred_rot = self.linear_rot.batch_generate(ctx_emb, trans_emb)
			...
			raise ValueError("Unsupported dimension in generate_accelerate")
