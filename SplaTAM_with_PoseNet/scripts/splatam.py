...
def get_dataset(config_dict, basedir, sequence, **kwargs):
    """RGB-D image sequences; standard SplatSLAM"""
...
def get_pointcloud(color, depth, intrinsics, w2c, transform_pts=True, 
    ...
    """Converts RGB-D image into a 3D point cloud; standard SplatSLAM"""
    ...
    if transform_pts: ### Optionally transform points into world coordinates
    ...
    # Compute mean squared distance for initializing the scale of the Gaussian splats
    ...
    # Select points based on mask - remove unwanted points
    if mask is not None:
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, use_posenet=False):
    """Initializes 3D Gaussians for Splat-SLAM; initialised for PoseNet"""
...
def initialize_optimizer(params, lrs_dict, tracking, transNet=None, rotsNet=None):
    """Optimiser for SLAM; optimised PoseNet"""
    lrs = lrs_dict
    if tracking:
...
def initialize_camera_pose(params, curr_time_idx, forward_prop):
    """Predicts camera pose for a new frame; Predicted using PoseNet"""
    with torch.no_grad():
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        # LOCALISATION
        # Initialize the camera pose for the current frame
        ### Calculates the likely next pose based on velocity between past two frames
        if time_idx > 0:
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']: ###Uses PoseNet
            ...
            while True:
                ...
                # print("trans_delta", trans_delta) #XXX print this
                ...
                ## Loss for current frame - RGB loss (difference between real and rendered image) and depth loss
                ### rendered image: as if a camera with the camera intrinsics and estimated pose had taken a photo of the gaussian splats map
                ...
                # Check if we should stop tracking based on min loss threshold
                iter += 1
        ...
        elif time_idx > 0 and config['tracking']['use_gt_poses']: ###Standard SplatSLAM (discrete poses instead of PoseNet)
            with torch.no_grad():
        ...
        # MAPPING - original SplatSLAM (but PoseNet camera poses)
        # Densification & KeyFrame-based Mapping
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            # Densification
            ### Adds new Gaussians to the scene if new points (e.g., previously occluded areas) become visible
            ...
            with torch.no_grad():
                ...
                curr_w2c[:3, :3] = build_rotation(curr_cam_rot)
                curr_w2c[:3, 3] = curr_cam_tran
                # Select Keyframes for Mapping, based on depth overlap (provide good view of scene) and pose similarity (different enough to have useful info) - using all frames is inefficient
            ...
            # Actual Mapping
            ### Periodically selects keyframes, optimizes the scene representation, and improves the reconstructed map.
            ...
            for iter in range(num_iters_mapping):
                ...
                ### Select one of the previous (or the current) keyframes for optimisation - use all past keyframes to capture dependencies and avoid overfitting to recent
                ...
                # Loss for current frame - RGB loss, Depth loss, silhoutte loss (new objects captured)
