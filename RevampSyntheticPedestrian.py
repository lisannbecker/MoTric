...
def generate_noisy_past_clean_future_dataset(timesteps, past_window_size, future_window_size,
    ...
    for t in range(past_window_size):
        # Get the GT pose
        ...
        if noise_type == 'random_independent':
            ...
            if t == 0:
                ...
                accumulated_rot_noise = np.eye(3)  # Identity rotation
...
if __name__ == "__main__":
    ...
    #                        curve_radius=5.0, # radius of curve in meters (if selected right curve)
    #                        right_bias_factor=3 # magnitude of right bias (if selected right bias)
    #                        )
    ...
    SE3_poses_gt, SE3_poses_noisy = generate_noisy_past_clean_future_dataset(timesteps, past_window_size, future_window_size,
                                            trans_noise_level=0.02, 
                                            rot_noise_level=1.0,
                                            step_size_x=0.1, 
                                            step_size_y=0.02,
    ...
    plot_noisy_vs_gt_trajectory(
        ...
        SE3_poses_noisy, 
        past_window_size,
        os.path.join(output_dir, "Straight_RandomWalk.png")
    )