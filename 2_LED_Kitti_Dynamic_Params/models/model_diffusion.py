...
class social_transformer(nn.Module):
	def __init__(self, t_h: int):
		...
		self.encode_past = nn.Linear(t_h*6, 256, bias=False) #LB need to update XXX
...
class TransformerDenoisingModel(Module):
	...
	def __init__(self, t_h: int, context_dim=256, tf_layer=2):
		super().__init__()
		self.encoder_context = social_transformer(t_h)
