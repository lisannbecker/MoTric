...
if __name__ == "__main__":
    ...
    random_se3 = 0.15*torch.rand(config['max_time'], 6) #randomly sampled trajectory with smoothness contstraints [100, 6] - PosNet learns 1x6 "twist" [ω_x, ω_y, ω_z, t_x, t_y, t_z]
    ...
    for i in range(train_iters):
        ...
        # dummy loss 
        loss = torch.abs(random_SE3 - est_c2ws).mean() # L1 loss: compare predicted poses (est_c2ws) to poses true poses (random_SE3 - poses sampled from a smooth trajectory)
                                                        # predicted poses are predicted from time alone
...
#LB Training loop
# aria_poses = load_aria_dataset()  # load aria data - returns SE(3) matrices - GT

# random_se3 = se3_from_SE3(aria_poses)  # convert to Lie representation [1, 6] so [N, 6]

# random_SE3 = lie_instance.se3_to_SE3(random_se3) #random_SE3 is reconstructed from PoseNet’s predictions
