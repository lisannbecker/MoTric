...
if __name__ == "__main__":
    ...
    SE3_kitti = load_kitti_poses(traj_idx)
    n_trajs = SE3_kitti.size()[0]
    config['max_time'] = n_trajs

    #se3_kitti = lie_instance.SE3_to_se3(SE3_kitti)
    
    posenet = PoseNet(config) #initialise PoseNet
    train_iters = 200

    if kitti_time == False:
        times = torch.arange(config['max_time']) #uniform timesteps that are simply ordered
    else:
        times = load_kitti_times(traj_idx)
    ...
    for i in range(train_iters):
        ...
        # dummy loss 
        loss = torch.abs(SE3_kitti - est_c2ws).mean() # L1 loss: compare se3 sampled from a smooth trajectory (GT) AND estimated se3 based on time alone
        ...
        if i in [1, train_iters-1]:
            # print(f'SE3_kitti: {SE3_kitti.size()},\n{SE3_kitti[0]}')
